      ...
      <div class="search">
        <form>
          <div class="search-text">
            <input type="text" v-model="txt" placeholder="搜索我的委托" />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      showflag: false,
      showtest: false,
      tasks:[{user1: '',title: '',content: '',repay: '',user2: ''}],
      i: 0,
    }
  },
  methods: {
  ...
  created:function () {
    ...
    if(this.$store.state.username == '') {
      ...
      var task = {user2: that.$store.state.username}
      axios
          .post("http://xcyzz.vipgz4.idcfengye.com/homepage/get", task)
          .then(function (response) {
            that.tasks = response.data;
            if(response.data.length == 0) {
              that.tasks[0] = {user1: '无',title: '暂时没有委托',content: '无',repay: '0'}
            }
          })
          .catch(function (error) {
            console.log(error)
